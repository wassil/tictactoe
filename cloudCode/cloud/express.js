...
app.all('/fb', function(req, res) {
	if (req.body && req.body['signed_request']) {
		...
		if (srJSON.user_id) {
			...
			if (req.query.fb_source == "syncrequest") {
				redirect += "&fb_source=syncrequest&request_id=" + req.query.request_id;
			}
			res.redirect(redirect);
		} else {
			var login_url = 'https://www.facebook.com/dialog/oauth?client_id='+Config.APP_ID+'&redirect_uri='+escape(Config.APP_URI+"?request_id="+req.query.request_id)+'';
